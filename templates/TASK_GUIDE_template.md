...
**Consumer** (required for any task that adds or changes a backend route, or a persisted field a user is expected to set): name the frontend surface that reaches it — a page, component, or control — **or** `UI deferred to Txxx` naming a registered task, **or** `Intentionally headless: <reason>`.
> Example: `Consumer: SettingsPage kitchen-rename form` or `Consumer: UI deferred to T048` or `Consumer: Intentionally headless: cron-invoked, no user surface`
>
> **This line may not be left blank on a backend task, and a blank blocks Done.** It exists because
> this project has shipped the same defect seven times (T031/T032/T033/T035/T036/T039/T041): a
> backend task completes correctly within its own scope, no downstream task is ever scheduled to
> build the consumer, and the feature is silently unreachable. Nothing fails and CI stays green —
> see `docs/audits/backend-frontend-coverage_2026-07-28.md`, which found 9 such routes at once.
> Seed data often masks it, so "it works in the demo" is not evidence a real user can reach it.
