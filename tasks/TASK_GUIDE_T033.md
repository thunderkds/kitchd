...
|-------|--------|------------------------|
| **New test(s) cover Acceptance Criteria (file paths pasted)** | ☑ pass | `apps/web/src/features/announcements/AnnouncementsPage.test.tsx` (9 tests: history render AC1, Owner/Chef form visible AC2/AC4, Admin form hidden + createAnnouncement never called AC2/AC5, Staff form hidden, broadcast flow updates list without reload AC3, read-receipt count AC4, empty state, error-dialog AC6) + `apps/web/src/App.test.tsx` (route wiring, placeholder-exclusion update, no regression to other nav items) |
| Verification command run | ☑ pass | `cd apps/web && npm test -- Announcements` → `Test Files 2 passed (2), Tests 12 passed (12)` (includes pre-existing AnnouncementsWidget.test.tsx, untouched, still green) |
| Negative cases hold | ☑ pass | Admin: form absent, `createAnnouncement` spy never invoked even though list still renders (fetch itself gated at the write-call level per AC5, list fetch stays open since `GET /announcements` has no `@Roles` guard server-side); Staff: form absent |
| verify | ☐ N/A | Not run live end-to-end (no Playwright browser session / seeded `.env` set up in this worktree at task start — component-level Testing Library coverage + `npx tsc --noEmit` clean + `oxlint` clean substituted; no fabricated live-browser evidence). Flagging for Supervisor to decide whether a live pass is required before merge. |
| Review scope bounded to the change's blast radius | ☑ pass | Touched only `apps/web/src/features/announcements/**` (new: api.ts, types.ts, AnnouncementsPage.tsx, AnnouncementsPage.test.tsx) + `apps/web/src/App.tsx` (route wire, additive `if` branch only) + `apps/web/src/App.test.tsx` (placeholder-exclusion list update, matching existing T026/T028 pattern). `AnnouncementsWidget/**` untouched as required. |
| Full smoke suite still green (no regression) | ☑ pass | `npm test` (apps/web) → `Test Files 24 passed (24), Tests 111 passed (111)` — no regressions in Tasks/Notes/Dashboard/Settings/Team suites |
| **UI: Visual regression** | ☐ N/A | Not captured (Playwright/easy-ui-mcp not exercised this pass — see `verify` row); page reuses the same Tailwind classes/patterns as TeamPage/NotesPage/AnnouncementsWidget (border, `bg-surface-raised`, `rounded-lg`, `divide-y`) so no new visual surface was introduced |
| **UI: Design-system compliance** | ☑ pass | Reviewed by inspection: only semantic Tailwind tokens used (`text-primary`, `text-muted`, `text-danger`, `bg-surface-raised`, `bg-primary`, `text-on-primary`) — no raw hex/rgb values, consistent with AnnouncementsWidget/TeamPage/NotesPage patterns |
| **UI: Responsiveness** | ☑ pass | `max-w-2xl mx-auto` container caps desktop width and centers; no fixed pixel widths; form fields and list use `w-full`/flex column layout that reflows naturally at mobile/tablet widths — same responsive pattern as NotesPage/TeamPage, not screenshot-verified live this pass |
