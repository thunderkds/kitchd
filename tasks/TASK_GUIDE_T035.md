...
|-------|--------|------------------------|
| **New test(s) cover Acceptance Criteria (file paths pasted)** | ☒ pass | `apps/web/src/features/tasks/TasksPage.test.tsx` (AC1 create-button RBAC, AC2/AC8 plain create + blank-title validation, AC5 source-recipe title resolution, plus all pre-existing kanban/list/T011 cases re-verified against the new 3-way fetch routing); `apps/web/src/features/tasks/CompleteTaskDialog/CompleteTaskDialog.test.tsx` (AC6 ingredient-name display + fallback-to-id-on-lookup-miss). `npx vitest run src/features/tasks` → 3 files, 18/18 pass. |
| Verification command run | ☒ pass | `cd apps/web && npx vitest run src/features/tasks` → `Test Files 3 passed (3)  Tests 18 passed (18)`. Full FE suite also re-run: `npx vitest run` → `Test Files 27 passed (27)  Tests 142 passed (142)`. `npx tsc --noEmit` clean. `npm run lint` (oxlint) clean — no new warnings from T035 files. |
| Negative cases hold | ☒ pass | AC8 (blank-title client-side block, no `POST /tasks` fires — asserted via before/after POST-call-count diff) covered in `TasksPage.test.tsx`; AC6 ingredient-lookup-miss fallback covered in `CompleteTaskDialog.test.tsx`; live curl probe confirmed Staff gets `403` from `POST /tasks` at the API layer (RBAC not weakened by the new UI). |
| verify | ☒ pass | Live API verify (curl, `apps/api` running via `npm run start:dev` against seeded demo data): logged in as `owner@demo.kitchenos.dev`; `POST /tasks` plain create → 200 with new Task; `POST /tasks/generate-from-recipe/recipe/:id` → 200, `sourceRecipeId` set, checklist snapshotted from recipe steps; `POST /tasks/generate-from-recipe/guideline/:id` → 200, `sourceGuidelineId` set; `GET /ingredients` confirmed name lookup data available for CompleteTaskDialog; Staff `POST /tasks` → `403`. No browser/easy-ui-mcp session was available inside this sub-agent's toolset, so the UI-evidence rows below are backed by component tests + the live API probes above, not a captured screenshot — flagging for Supervisor to optionally re-run `easy-ui-mcp` live if a screenshot is required before merge. |
