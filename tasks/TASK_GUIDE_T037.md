...
| verify | ☒ pass | Supervisor live browser session (easy-ui-mcp): Owner — Inventory "Add Ingredient" opens modal (confirmed `role="dialog"` + "New Ingredient" title); per-row "Edit" opens a modal pre-filled with that row's name (no more inline row-edit); Guidelines "New Guideline" opens modal with sidebar/nav still visible behind it (page no longer replaced). Staff — confirmed no "Add Ingredient"/"Edit" buttons on Inventory (write access correctly gated); confirmed "New Note" button still visible on Notes (unrestricted-by-design RBAC preserved exactly). Report archived at `reports/evidence/T037/session-b59d6dbf-ea96-4373-864b-68ad2fb5a11e.{json,html}`. |
...
| **UI: Visual regression** | ☒ pass | Live-verified: Inventory create/edit modals and Guidelines create modal all render through the shared `Dialog` primitive with the same title / Cancel(border) / Submit(bg-accent) button layout as `CreateTaskDialog`. No per-page bespoke chrome introduced. |
| **UI: Design-system compliance** | ☒ pass | No raw hex added; buttons/labels reuse `bg-accent`, `text-danger`, `text-muted`, `bg-surface`, `border` tokens already used by `CreateTaskDialog.tsx`. |
| **UI: Responsiveness at target viewports** | ☒ pass | All modals inherit `Dialog`'s `w-full max-w-md p-4 sm:p-6` responsive shell unchanged (already verified at mobile/tablet/desktop breakpoints in T021's mobile responsive pass); no per-page bespoke sizing added. |
