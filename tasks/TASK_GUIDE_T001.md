...
| Full smoke suite still green (no regression) | ✅ pass | No pre-existing suite; this is the first test suite in the repo. `npm --prefix apps/api run test` → 3 suites / 11 tests, all green. `npm --prefix apps/web run lint` (oxlint) and `npm --prefix apps/web run build` both clean. `npm --prefix apps/api run lint` (eslint --fix) clean. |
