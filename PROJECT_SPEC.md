...
| T027 | Backend team management (member list, role-change, remove, invite management) | Done | backend-developer | C2 | High | P1 |
| T028 | Frontend Team & Roles page | Done | frontend-developer | C2 | Medium | P1 |
...
| 2026-07-14 | **T025 Done** — Backend theme-preference (`User.themePreference` enum + `PATCH /users/me/theme`) merged to develop (merge commit `2d2add9`, feature commit `50c558d`). Additive migration (GO), code-review 0 P0/P1/P2/P3, 5 new e2e tests + full 186-test suite green post-merge. No `/auth/me` route exists in this codebase — `themePreference` is surfaced via the `user` object already returned by `/auth/signup` and `/auth/login`. Unblocks T026. | T025 |
| 2026-07-14 | **T026 Done** — Frontend theme system (semantic CSS-variable tokens, `[data-theme]` blocks, `ThemeProvider` with anti-flash + optimistic switching, kebab/snake_case casing boundary, Settings-page switcher, all 18 raw-Tailwind-color files migrated) merged to develop. Code-review 0 findings, 73/73 frontend tests green post-merge, UI evidence archived at `reports/evidence/T026/`. Closes the T025/T026 theme-system milestone (US-014/FR-025). | T026 |
| 2026-07-14 | **T027 Done** — Backend team management (`User.isActive` soft-deactivation, `InviteStatus.REVOKED`, 4 new Owner/Admin-only endpoints: `GET /users`, `PATCH /users/:id/role`, `DELETE /users/:id`, `GET/DELETE /users/invites`) merged to develop. migration-safety GO, code-review 0 P0/1 P1(accepted), security-review 0 High/1 Medium(accepted) — both flag the same tracked gap: a deactivated user's JWT remains valid until natural expiry since `roles.guard.ts` wasn't touched (deliberate scope boundary). 202/202 tests green post-merge. Unblocks T028. | T027 |
| 2026-07-14 | **T028 Done** — Frontend Team & Roles page (member roster, role-change, remove, invite, revoke, all Owner/Admin-gated with zero fetches for non-managers) merged to develop. Code-review 0 P0/P1 (1 P2 advisory: controls not disabled during in-flight requests), 82/82 frontend tests green post-merge, UI evidence archived at `reports/evidence/T028/`. Closes the T027/T028 team-management milestone (US-015/FR-026). Surfaced a real backend bug in T027's `revokeInvite` (500 on re-inviting a previously-revoked email) — tracked as a follow-up, not fixed in T028's scope. | T028 |
| 2026-07-14 | Theme system direction locked (Option B — semantic CSS-variable tokens + `data-theme` attribute, see `BRAINSTORMING_LOG_theme-system.md` and `docs/adr/0001-theme-system-token-architecture.md`): "Simple"/"Dark Neon" themes, single `--color-accent` token (no split primary/secondary — no current usage justifies it), theme IDs are kebab-case `"simple"`/`"dark-neon"`, persistence is account-synced via a new Prisma **enum** `User.themePreference` (not a plain String) matching the existing `Role` enum convention — adding a future theme requires a migration by design. | Stage 0.5/2 (brainstorming + grill-with-docs) |
